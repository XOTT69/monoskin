  ...
  return <main>
    <section className="hero" id="top">
      <nav className="nav container" aria-label="Головна навігація">
        ...
        <div className="nav-actions"><a href="#guide">Як додавати</a><a className="nav-catalog" href="#catalog">Каталог <span>↓</span></a></div>
      </nav>
    ...
    <section className="guide container" id="guide"><div><p className="eyebrow"><span /> Керування каталогом</p><h2>Новий скін —<br />за хвилину</h2><p>Дані та картинки живуть окремо від дизайну. Додай скільки завгодно скінів: пошук, фільтри й сторінка деталей працюватимуть автоматично.</p></div><ol><li><b>01</b><span>Додай картинку в <code>public/skin</code></span></li><li><b>02</b><span>Додай запис у <code>data/skins.json</code></span></li><li><b>03</b><span>Зроби push — GitHub Pages оновить сайт</span></li></ol><a className="text-link" href="https://github.com/XOTT69/monoskin/blob/main/data/skins.json" target="_blank" rel="noreferrer">Відкрити список скінів <span>↗</span></a></section>
    ...
    {selected && <div className="overlay" role="presentation" onMouseDown={() => setSelected(null)}><section className="details" role="dialog" aria-modal="true" aria-labelledby="details-title" onMouseDown={(event) => event.stopPropagation()}><button className="close" onClick={() => setSelected(null)} aria-label="Закрити деталі">×</button><div className="detail-art" style={imageStyle(selected)}><span className={`method ${methodClass[selected.method]}`}>{selected.method}</span></div><div className="detail-body"><p className="eyebrow"><span /> {selected.status}</p><h2 id="details-title">{selected.name}</h2><p className="detail-description">{selected.description}</p><div className="detail-meta"><span>{money(selected.minimumValue)}</span><span>{date(selected.date)}</span>{selected.isVisaOnly && <span>Лише Visa</span>}{selected.isAdultOnly && <span>18+</span>}</div><div className="condition"><span>Як отримати</span><p>{selected.status === "Недоступний" ? "Видачу скіна завершено. Зберігаємо запис як частину каталогу." : `Спосіб отримання: ${selected.method.toLowerCase()}.`}</p></div>{selected.sourceUrl ? <a className="primary-button detail-button" href={selected.sourceUrl} target="_blank" rel="noreferrer">Відкрити посилання <span>↗</span></a> : <span className="disabled-button">Посилання недоступне</span>}</div></section></div>}
  </main>;
}
