...
function imageFromClipboard(items: DataTransferItemList) {
  ...
  return new File([file], `вставлений-скін.${extension}`, { type: file.type });
}
...
async function imageFromSystemClipboard() {
  ...
  try {
    for (const clipboardItem of await navigator.clipboard.read()) {
      ...
      return new File([blob], `вставлений-скін.${type.split("/")[1] || "png"}`, { type });
    }
  } catch {
...
export default function AdminPage() {
  ...
  const setChosenPhoto = (selected: File | null) => {
